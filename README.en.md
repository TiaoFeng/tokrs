...
A local token usage statistics CLI written in Rust. It reads log and database files left on the local machine by Claude Code, Codex, OpenCode V2 (V1), Gemini CLI, Grok Build, Pi, Kimi Code, Deepseek Harness, and VS Code Copilot Chat to track token consumption and costs by app, model, and date. It has no daemon processes, makes no network requests, and accesses data sources in read-only mode.
...
- Supports 9 agents: Claude / Codex / OpenCode / Gemini / Grok / Pi / Kimi / Deepseek Harness / Copilot
...
|---|---|
| `--app <a,b,c>` | Count only the specified apps (comma-separated); available values: `claude`, `codex`, `opencode`, `gemini`, `grok`, `pi`, `kimi` `dsh` `copilot`; by default, count all |
...
| Copilot | `<user dir>/workspaceStorage/*/chatSessions/*.jsonl` (empty window: `<user dir>/globalStorage/emptyWindowChatSessions/*.jsonl`; user dir defaults to `~/.config/Code/User`) | VS Code chat session operation log counted as per-request `promptTokens`/`completionTokens` face values; deduplicated by `requestId`; no cache breakdown (input is `promptTokens`); extension model-id decorations (namespace/`::` variant) are stripped to align with the unified model names; Auto mode is resolved to the actual model via `autoModeResolution` (falls back to `auto` when absent) |
